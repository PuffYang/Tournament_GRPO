...
@ToolParser.register("tournament_grpo_xml")
@ToolParser.register("dr_tulu_xml")
class TournamentGRPOXMLToolParser(ToolParser):
    """Parser for TournamentGRPO XML tool calls.
    ...
    def __init__(self, tokenizer) -> None:
        super().__init__(tokenizer)
        self.call_tool_end_token = "</call_tool>"
        self.call_tool_regex = regex.compile(
            r'<call_tool\s+name="([^"]+)"([^>]*)>(.*?)</call_tool>',
            regex.DOTALL,
        )
        self.attr_regex = regex.compile(r'([A-Za-z_][A-Za-z0-9_]*)="([^"]*)"')

    def get_stop_strings(self) -> list[str]:
        return [self.call_tool_end_token]
    ...
    def _infer_primary_arg_name(tool_name: str, tools: Optional[list[OpenAIFunctionToolSchema]]) -> str:
        if tools:
            for tool in tools:
                if tool.type != "function" or tool.function.name != tool_name:
                    continue
