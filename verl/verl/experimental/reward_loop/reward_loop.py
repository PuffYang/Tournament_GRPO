...
class RewardLoopManager:
    ...
    def _chunk_by_uid_for_tournament(self, data: DataProto) -> list[tuple[list[int], DataProto, ActorHandle]]:
        if "uid" not in data.non_tensor_batch:
            raise ValueError("Tournament reward requires uid so all rollouts from the same prompt stay together.")

        uid_to_indices: dict[object, list[int]] = {}
        for idx in range(len(data)):
            uid = data[idx].non_tensor_batch["uid"]
            uid_to_indices.setdefault(uid, []).append(idx)

        prompt_ids = data.batch.get("prompts", None)
        if prompt_ids is not None:
            for uid, indices in uid_to_indices.items():
                first_prompt = prompt_ids[indices[0]]
                if any(not torch.equal(first_prompt, prompt_ids[idx]) for idx in indices[1:]):
                    raise ValueError(f"Tournament uid group {uid} contains different prompts.")

        worker_count = max(1, min(len(self.reward_loop_workers), len(uid_to_indices)))
        buckets: list[list[int]] = [[] for _ in range(worker_count)]
        bucket_sizes = [0] * worker_count

        for indices in uid_to_indices.values():
            bucket_idx = min(range(worker_count), key=lambda idx: bucket_sizes[idx])
            buckets[bucket_idx].extend(indices)
            bucket_sizes[bucket_idx] += len(indices)

        chunks = []
        for worker_idx, indices in enumerate(buckets):
            if not indices:
                continue
            chunks.append((indices, data.select_idxs(indices), self.reward_loop_workers[worker_idx]))
        return chunks
    ...
    def compute_rm_score(self, data: DataProto) -> DataProto:
        ...
        if self.config.reward.reward_manager.name == "tournament":
            chunks = self._chunk_by_uid_for_tournament(data)
            chunk_outputs = ray.get([worker.compute_score_batch.remote(chunk) for _, chunk, worker in chunks])
            outputs_flat = [None] * len(data)
            for (indices, _, _), outputs in zip(chunks, chunk_outputs, strict=True):
                for global_idx, output in zip(indices, outputs, strict=True):
                    outputs_flat[global_idx] = output
            outputs_flat = [
                output if output is not None else {"reward_score": 0.0, "reward_extra_info": {}} for output in outputs_flat
            ]
        else:
