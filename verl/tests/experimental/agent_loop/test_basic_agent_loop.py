...
def test_postprocess_preserves_zeroed_tool_metrics_for_concat():
    worker = AgentLoopWorker.__new__(AgentLoopWorker)
    worker.reward_loop_worker_handles = None

    def make_output(tool_call_counts=None, google_search_calls=None, browse_webpage_calls=None):
        extra_fields = {}
        if tool_call_counts is not None:
            extra_fields["tool_call_counts"] = tool_call_counts
        if google_search_calls is not None:
            extra_fields["google_search_call_count"] = google_search_calls
        if browse_webpage_calls is not None:
            extra_fields["browse_webpage_call_count"] = browse_webpage_calls

        return _InternalAgentLoopOutput(
            prompt_ids=torch.tensor([[1, 2]], dtype=torch.int64),
            response_ids=torch.tensor([[3, 4]], dtype=torch.int64),
            input_ids=torch.tensor([[1, 2, 3, 4]], dtype=torch.int64),
            position_ids=torch.tensor([[0, 1, 2, 3]], dtype=torch.int64),
            response_mask=torch.tensor([[1, 1]], dtype=torch.int64),
    ...
    no_tool_chunk = worker._postprocess([make_output(), make_output()])
    tool_chunk = worker._postprocess(
        [
            make_output(tool_call_counts=1, google_search_calls=1),
            make_output(tool_call_counts=2, browse_webpage_calls=2),
        ]
    )

    merged = DataProto.concat([no_tool_chunk, tool_chunk])

    assert merged.batch.batch_size[0] == 4
    assert merged.non_tensor_batch["tool_call_counts"].tolist() == [0, 0, 1, 2]
    assert merged.non_tensor_batch["google_search_call_count"].tolist() == [0, 0, 1, 0]
    assert merged.non_tensor_batch["browse_webpage_call_count"].tolist() == [0, 0, 0, 2]
